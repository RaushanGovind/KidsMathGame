...
function initFractionsMode() {
    ...
    document.getElementById("additionColumns").innerHTML = "";
    document.getElementById("bubbleMatrix").style.display = "none";
    ...
    // Setup popup if not already done
    if (!window.fractPopupSetupDone) {
        setupFractPopup();
        window.fractPopupSetupDone = true;
    }
}
...
// Global hooks
window.initFractions = initFractionsMode;
...
/* =========================================================
   FRACTION POPUP CONTROLS
========================================================= */

function openFractPopup() {
    const popup = document.getElementById("fractPopup");
    if (popup) popup.style.display = "flex";
}

function closeFractPopup() {
    const popup = document.getElementById("fractPopup");
    if (popup) popup.style.display = "none";
}

function setupFractPopup() {
    console.log("Setting up Fraction Popup...");

    // Close button
    const closeBtn = document.querySelector("#fractPopup .popup-close-btn");
    if (closeBtn) closeBtn.onclick = closeFractPopup;

    const cancelBtn = document.getElementById("fractCancel");
    if (cancelBtn) cancelBtn.onclick = closeFractPopup;

    // Mode Selection (Learn vs Question)
    const modeBtns = document.querySelectorAll('.fract-mode-btn');
    modeBtns.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            console.log("Fraction Mode selected:", mode);
        });
    });

    // Shape Selection
    const shapeBtns = document.querySelectorAll('.fract-shape-btn');
    shapeBtns.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            setFractionShape(shape);
            console.log("Fraction Shape selected:", shape);
        });
    });

    // Start Button (closes popup)
    const startBtn = document.getElementById("fractStart");
    if (startBtn) {
        startBtn.onclick = () => {
            closeFractPopup();
            // If Question Mode is selected, we might want to regenerate a question
            if (fractionState.mode === 'question') {
                startFractionQuestion();
            }
        };
...
window.openFractPopup = openFractPopup;
window.closeFractPopup = closeFractPopup;
