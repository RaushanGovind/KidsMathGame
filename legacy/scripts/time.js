...
function updateClock(h, m) {
    ...
    // Label update
    const text = getTimeAsText(h, m);
    document.getElementById("timeLabel").innerText = text;

    // Speak Time ONLY in Learn Mode (not Question Mode initially)
    if (timeState.mode === 'learn' && window.speechSynthesis) {
        window.speechSynthesis.cancel();
        const msg = new SpeechSynthesisUtterance("It is " + text);
        msg.rate = 1.0;
...
function checkTimeAnswer(btn, selected, correct) {
    if (selected === correct) {
        ...
        if (window.speechSynthesis) {
            window.speechSynthesis.cancel();
            const msg = new SpeechSynthesisUtterance("Correct! It is " + selected);
            msg.rate = 1.1;
            msg.pitch = 1.2;
            window.speechSynthesis.speak(msg);
        }
        ...
        if (window.speechSynthesis) {
            window.speechSynthesis.cancel();
            // Maybe just say the number they clicked?
            const msg = new SpeechSynthesisUtterance(selected + " ... Try again.");
            msg.rate = 1.1;
            window.speechSynthesis.speak(msg);
        }
