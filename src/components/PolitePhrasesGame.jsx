...
function PolitePhrasesGame({ onBack }) {
    ...
    useEffect(() => {
        fetch('http://localhost:8000/api/content/polite_phrases')
            .then(res => res.json())
            .then(data => {
            ...
            .catch(err => {
                console.error("Failed to load polite phrases content", err);
                setLoading(false);
            });
    }, []);
    ...
    const startQuizRound = () => {
        ...
        setTimeout(() => speak(`Identify the phrase: ${target.text}`), 500);
    };
    ...
    if (loading) return <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '2rem' }}>Loading Polite Phrases...</div>;
        ...
        <div className="game-container" style={{
            ...
            <div style={{ width: '100%', maxWidth: '1000px', display: 'flex', justifyContent: 'space-between', marginBottom: '20px', alignItems: 'center' }}>
                ...
                <div style={{ display: 'flex', gap: '15px' }}>
                    <button onClick={() => setMode('learn')} style={{ padding: '10px 20px', background: mode === 'learn' ? '#FBC02D' : 'white', color: mode === 'learn' ? 'white' : '#2C3E50', borderRadius: '20px', border: 'none', fontWeight: 'bold', cursor: 'pointer' }}>LEARN</button>
                    <button onClick={() => setMode('quiz')} style={{ padding: '10px 20px', background: mode === 'quiz' ? '#7B1FA2' : 'white', color: mode === 'quiz' ? 'white' : '#2C3E50', borderRadius: '20px', border: 'none', fontWeight: 'bold', cursor: 'pointer' }}>QUIZ</button>
                </div>
            </div>
            ...
            {mode === 'learn' ? (
                <div style={{ display: 'flex', alignItems: 'center', gap: '20px', width: '100%', maxWidth: '1000px', justifyContent: 'center' }}>
                    ...
                    <motion.div
                        ...
                        style={{
                            ...
                            display: 'flex', flexDirection: 'column', alignItems: 'center',
                            border: '6px solid #FBC02D', position: 'relative', cursor: 'pointer'
                        }}
                    >
                        ...
                        <div style={{ fontSize: '3.5rem', fontWeight: '1000', color: '#F9A825', lineHeight: 1.2, marginBottom: '20px', textAlign: 'center' }}>
                            {currentItem.text}
                        </div>

                        <div style={{ fontSize: '2rem', fontWeight: '700', color: '#F57F17', background: '#FFF9C4', padding: '15px 30px', borderRadius: '20px', textAlign: 'center' }}>
                            "{currentItem.context}"
                        </div>
                    </motion.div>
                ...
                <div style={{ width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                    ...
                    <h2 style={{ fontSize: '2rem', marginBottom: '30px', color: '#FBC02D' }}>Identify: "{quizTarget?.text}"</h2>
                    <div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap', justifyContent: 'center' }}>
                        {quizOptions.map((item, idx) => (
                            <motion.button key={idx} whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }} onClick={() => handleQuizOptionClick(item)} style={{ background: 'white', border: '4px solid #FBC02D', borderRadius: '20px', padding: '30px 40px', display: 'flex', flexDirection: 'column', alignItems: 'center', boxShadow: '0 8px 0 #F9A825', cursor: 'pointer', minWidth: '220px' }}>
