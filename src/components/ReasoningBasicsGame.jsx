...
function ReasoningBasicsGame({ onBack }) {
    ...
    useEffect(() => {
        fetch('http://localhost:8000/api/content/reasoning_basics')
            .then(res => res.json())
            .then(data => {
                setGameData(data);
                setLoading(false);
                // Set initial category if data is loaded
    ...
    return (
        <div className="game-container" style={{
            ...
            <div style={{ width: '100%', maxWidth: '1000px', display: 'flex', justifyContent: 'space-between', marginBottom: '20px', alignItems: 'center' }}>
                ...
                <div style={{ fontSize: '2rem', fontWeight: '900', color: '#E67E22', display: 'flex', alignItems: 'center', gap: '10px' }}>
                    <span>🧠</span> REASONING BASICS
                </div>
            </div>
                    ...
                    >
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '20px' }}>
                            {currentData.content.map((item, idx) => (
                                <div key={idx}
                                    onClick={() => speak(`${item.text} ... Answer is ${item.answer}`)}
                                    style={{
                                    ...
                                    <div style={{ fontSize: '1.5rem', fontWeight: 'bold', color: '#2C3E50', marginBottom: '15px', lineHeight: '1.4' }}>{item.text}</div>
                                    <div style={{ fontSize: '1.3rem', color: currentData.color, fontWeight: 'bold' }}>👉 {item.answer}</div>
                                </div>
                            ))}
                        </div>
                    </motion.div>
...
export default ReasoningBasicsGame;
