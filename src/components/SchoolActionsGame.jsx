...
function SchoolActionsGame({ onBack }) {
    ...
    const currentAction = SCHOOL_ACTIONS[currentIndex];
    ...
    const nextAction = () => {
        if (currentIndex < SCHOOL_ACTIONS.length - 1) setCurrentIndex(c => c + 1);
    };
    ...
    const startQuizRound = () => {
        const target = SCHOOL_ACTIONS[Math.floor(Math.random() * SCHOOL_ACTIONS.length)];
        setQuizTarget(target);
        setFeedback(null);
        const options = [target];
        while (options.length < 3) {
            const random = SCHOOL_ACTIONS[Math.floor(Math.random() * SCHOOL_ACTIONS.length)];
        ...
        <div className="game-container" style={{
            ...
            {mode === 'learn' ? (
                <div style={{ display: 'flex', alignItems: 'center', gap: '20px', width: '100%', maxWidth: '1000px', justifyContent: 'center' }}>
                    ...
                    >
                        <div style={{ position: 'absolute', top: '20px', right: '20px', fontSize: '1.5rem', color: '#95A5A6', fontWeight: 'bold' }}>{currentIndex + 1} / {SCHOOL_ACTIONS.length}</div>
                    ...
                    <button onClick={nextAction} disabled={currentIndex === SCHOOL_ACTIONS.length - 1} style={{ background: currentIndex === SCHOOL_ACTIONS.length - 1 ? '#ccc' : 'white', border: 'none', borderRadius: '50%', width: '60px', height: '60px', fontSize: '2rem', cursor: currentIndex === SCHOOL_ACTIONS.length - 1 ? 'default' : 'pointer', boxShadow: '0 4px 0 rgba(0,0,0,0.1)' }}>➡</button>
                </div>
            ) : (
